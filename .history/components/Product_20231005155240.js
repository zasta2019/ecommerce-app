   ...
   // navigation.navigate('Mycart');
    ...
    try {
      ...
      if (user) {
        ...
        const userId = parsedUser.data.id; // Access the 'id' property directly
        console.log(userId);
        ...
        if (cart.status === 200) {
          ...
          console.log('Failed to add to cart:', cart); // Log the response for debugging
        }
      } else {
        console.log('User is not logged in.'); // Handle the case where the user is not logged in
      }
    } catch (error) {
      console.log('Error adding to cart:', error); // Log any errors for debugging
    }
  };
